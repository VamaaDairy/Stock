...
export default function DatePeriodSelector({
  ...
  return (
    <div className="bg-white p-3 rounded-xl border border-black space-y-2 text-black">
      {/* Mode & Preset Buttons */}
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-1 border border-black rounded-lg p-0.5 bg-neutral-50">
          <Button
            type="button"
            variant="ghost"
            size="sm"
            ...
            className={`h-7 text-xs font-bold px-2.5 rounded-md ${
              mode === "single" ? "bg-black text-white" : "text-black hover:bg-neutral-200"
            }`}
          >
            Single Date
          </Button>
          <Button
            ...
            className={`h-7 text-xs font-bold px-2.5 rounded-md ${
              mode === "range" ? "bg-black text-white" : "text-black hover:bg-neutral-200"
            }`}
          >
            Date Range (Period)
          </Button>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          <Button
            ...
            className="h-7 text-[11px] font-bold px-2 border-black text-black hover:bg-neutral-100"
          >
            Today
          </Button>
          <Button
      ...
      {mode === "single" ? (
        <div className="flex items-center gap-2">
          ...
          <Input
            type="date"
            className="h-9 text-sm w-44 font-semibold border-black text-black bg-white"
            value={value.date}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1.5">
            <Label className="text-xs font-bold text-black uppercase">From:</Label>
            <Input
              type="date"
              className="h-9 text-sm w-36 font-semibold border-black text-black bg-white"
              value={value.fromDate}
              onChange={e => onChange({ ...value, mode: "range", fromDate: e.target.value })}
            />
          </div>
          <div className="flex items-center gap-1.5">
            <Label className="text-xs font-bold text-black uppercase">To:</Label>
            <Input
              type="date"
              className="h-9 text-sm w-36 font-semibold border-black text-black bg-white"
              value={value.toDate}
