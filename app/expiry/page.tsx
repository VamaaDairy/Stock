"use client"

import React, { useState, useEffect, useCallback, useMemo, Fragment } from "react"
import { Input } from "@/components/ui/input"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Search, AlertTriangle, Filter, ChevronRight, ChevronDown, FileSpreadsheet, RefreshCw } from "lucide-react"
import { PageHeader } from "@/components/ui/page-header"
import { formatMixedUnit, calcUBDPercent, ubdPercentColor } from "@/lib/utils"
import { exportToExcel } from "@/lib/excel-export"
import type { ExpiredStockItem } from "@/lib/db/expiry"

interface ExpiredProductGroup {
  id: string
  name: string
  skuCode: string
  category: string
  unit: string
  packLabel: string
  pcsPerCrt: number
  totalExpiredPcs: number
  expiredDisplay: string
  batches: ExpiredStockItem[]
}

export default function ExpiryPage() {
  const [items, setItems] = useState<ExpiredStockItem[]>([])
  const [loading, setLoading] = useState(true)
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedBatch, setSelectedBatch] = useState<string>("all")
  const [expandedProductIds, setExpandedProductIds] = useState<Set<string>>(new Set())
  const [refreshTrigger, setRefreshTrigger] = useState(0)

  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch("/api/stock/expiry")
      const json = await res.json()
      if (json.success) {
        setItems(json.data || [])
      }
    } catch (e) {
      console.error("Failed to load expired stock:", e)
    } finally {
      setLoading(false)
    }
  }, [])

  useEffect(() => {
    fetchData()
  }, [fetchData, refreshTrigger])

  const toggleProductExpand = (id: string) => {
    setExpandedProductIds((prev) => {
      const next = new Set(prev)
      if (next.has(id)) next.delete(id)
      else next.add(id)
      return next
    })
  }

  // Extract all unique batch numbers available across expired items
  const availableBatches = useMemo(() => {
    const set = new Set<string>()
    items.forEach((item) => {
      if (item.batchNumber) set.add(item.batchNumber.trim())
    })
    return Array.from(set).sort()
  }, [items])

  // Group items by Product
  const productGroups = useMemo(() => {
    const map = new Map<string, ExpiredProductGroup>()

    items.forEach((item) => {
      if (!map.has(item.productId)) {
        map.set(item.productId, {
          id: item.productId,
          name: item.productName,
          skuCode: item.skuCode,
          category: item.category,
          unit: item.unit,
          packLabel: item.packLabel,
          pcsPerCrt: item.pcsPerCrt || 1,
          totalExpiredPcs: 0,
          expiredDisplay: "",
          batches: [],
        })
      }
      const group = map.get(item.productId)!
      group.totalExpiredPcs += item.expiredQtyTotal
      group.batches.push(item)
    })

    // Calculate mixed unit display for each product group
    map.forEach((group) => {
      group.expiredDisplay = formatMixedUnit(
        group.totalExpiredPcs,
        group.pcsPerCrt,
        group.unit || "CRT",
        group.pcsPerCrt > 1 ? "PCS" : group.unit || "CRT"
      )
    })

    let list = Array.from(map.values())

    // Filter by Selected Batch
    if (selectedBatch !== "all") {
      list = list
        .map((p) => ({
          ...p,
          batches: p.batches.filter((b) => b.batchNumber === selectedBatch),
        }))
        .filter((p) => p.batches.length > 0)
    }

    // Filter by Search Query
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase().trim()
      list = list.filter(
        (p) =>
          p.name.toLowerCase().includes(q) ||
          p.skuCode.toLowerCase().includes(q) ||
          p.category.toLowerCase().includes(q) ||
          p.batches.some((b) => b.batchNumber.toLowerCase().includes(q))
      )
    }

    return list
  }, [items, selectedBatch, searchQuery])

  // Excel Export
  const handleExportExcel = () => {
    const headers = [
      "Category",
      "Product ID",
      "SKU Code",
      "Product Name",
      "Batch No",
      "MFD",
      "UBD",
      "UBD %",
      "Days Expired",
      "Unit",
      "Pcs/Crt",
      "Expired Qty (Mixed)",
      "Expired Qty (Total PCS)",
      "Status",
    ]

    const rows: (string | number | null | undefined)[][] = []

    productGroups.forEach((p) => {
      p.batches.forEach((b) => {
        const ubdVal = b.ubd || b.expiryDate || ""
        const pct = calcUBDPercent(ubdVal, b.shelfLifeDays, b.manufacturingDate)
        rows.push([
          p.category,
          p.id,
          p.skuCode,
          p.name,
          b.batchNumber,
          b.manufacturingDate || "",
          ubdVal,
          pct !== null ? `${pct}%` : "EXPIRED",
          `${b.daysExpired} days`,
          p.unit,
          p.pcsPerCrt,
          b.expiredQtyDisplay,
          b.expiredQtyTotal,
          "EXPIRED (Removed from Active Stock)",
        ])
      })
    })

    exportToExcel({
      filename: `Gaia_Expired_Stock_${new Date().toISOString().slice(0, 10)}`,
      sheetTitle: "Expired Stock Inventory Log",
      headers,
      rows,
    })
  }

  return (
    <div className="min-h-screen bg-white text-slate-800 px-4 py-6 pb-20">
      <div className="max-w-6xl mx-auto space-y-6">
        {/* Top Header */}
        <PageHeader
          icon={AlertTriangle}
          title="Expired Stock & Waste Log"
          subtitle="Batches that have reached or passed their Use Before Date (UBD). Automatically removed from active stock balance."
          actions={
            <div className="flex items-center gap-2 flex-wrap">
              <Button
                variant="outline"
                size="sm"
                onClick={handleExportExcel}
                disabled={items.length === 0}
                className="flex items-center gap-1.5 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 hover:text-emerald-800 border border-emerald-300 text-[11px] font-bold px-2.5 py-1 h-8 rounded-lg transition-colors shadow-2xs cursor-pointer"
              >
                <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                <span>Export Excel</span>
              </Button>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setRefreshTrigger((prev) => prev + 1)}
                className="h-8 px-2.5 text-[11px] font-semibold border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg cursor-pointer"
              >
                <RefreshCw className="w-3 h-3 mr-1" />
                Refresh
              </Button>
            </div>
          }
        />

        {/* Search & Batch Filter Toolbar */}
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            <Search className="absolute left-3.5 top-3.5 h-4 w-4 text-slate-800" />
            <Input
              placeholder="Search by Product Name, SKU Code (e.g. 1011) or Batch..."
              className="pl-10 h-11 bg-white text-base rounded-xl border-blue-200 text-slate-800 placeholder:text-slate-400"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          {availableBatches.length > 0 && (
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <span className="text-xs font-bold text-slate-500 uppercase whitespace-nowrap flex items-center gap-1">
                <Filter className="h-3.5 w-3.5" /> Batch:
              </span>
              <select
                value={selectedBatch}
                onChange={(e) => setSelectedBatch(e.target.value)}
                className="h-11 px-3 text-sm font-semibold rounded-xl border border-blue-200 bg-white text-slate-800 focus:outline-none"
              >
                <option value="all">All Batches ({availableBatches.length})</option>
                {availableBatches.map((b) => (
                  <option key={b} value={b}>
                    Batch: {b}
                  </option>
                ))}
              </select>
            </div>
          )}
        </div>

        {/* Expired Stock Table */}
        {loading ? (
          <div className="bg-white p-12 rounded-xl border border-blue-100 text-center text-slate-800">
            <p className="text-base font-semibold animate-pulse">Scanning and loading expired stock data...</p>
          </div>
        ) : (
          <div className="border border-blue-100 rounded-xl overflow-hidden bg-white shadow-xs">
            <table className="w-full text-left text-sm">
              <thead className="bg-gradient-to-r from-blue-50 to-slate-50 text-[#2B4C86] text-xs uppercase font-bold tracking-wider border-b border-blue-100">
                <tr>
                  <th className="py-3.5 px-3 w-10 text-center"></th>
                  <th className="py-3.5 px-4">SKU Code</th>
                  <th className="py-3.5 px-4">Product Name</th>
                  <th className="py-3.5 px-4">Category</th>
                  <th className="py-3.5 px-4 text-right font-black text-slate-800">Expired Quantity</th>
                  <th className="py-3.5 px-4 text-center">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-blue-100">
                {productGroups.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-10 text-center text-slate-400 font-medium">
                      No expired stock entries found.
                    </td>
                  </tr>
                ) : (
                  productGroups.map((p) => {
                  const isExpanded = expandedProductIds.has(p.id)
                  const hasBatches = p.batches.length > 0

                  return (
                    <Fragment key={p.id}>
                      {/* Primary Product Row */}
                      <tr
                        onClick={() => toggleProductExpand(p.id)}
                        className="hover:bg-blue-50/60 transition-colors cursor-pointer select-none"
                      >
                        <td className="py-3 px-3 text-center text-slate-400">
                          {isExpanded ? (
                            <ChevronDown className="h-4 w-4 mx-auto text-slate-800 font-bold" />
                          ) : (
                            <ChevronRight className="h-4 w-4 mx-auto text-neutral-400" />
                          )}
                        </td>
                        <td className="py-3 px-4 font-mono font-bold text-slate-800">
                          {p.skuCode ? (
                            <Badge variant="outline" className="border-blue-200 bg-blue-50/60 text-[#2B4C86] font-mono">
                              {p.skuCode}
                            </Badge>
                          ) : (
                            "—"
                          )}
                        </td>
                        <td className="py-3 px-4 font-bold text-slate-800">
                          <div className="flex items-center gap-2">
                            <span>{p.name}</span>
                            {hasBatches && (
                              <span className="text-[10px] text-slate-400 font-semibold bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100">
                                {p.batches.length} batch{p.batches.length > 1 ? "es" : ""}
                              </span>
                            )}
                          </div>
                        </td>
                        <td className="py-3 px-4 text-slate-500 font-semibold">{p.category}</td>
                        <td className="py-3 px-4 text-right font-black text-base text-slate-800">
                          <div className="font-extrabold text-slate-800">{p.expiredDisplay}</div>
                          {p.pcsPerCrt > 1 && (
                            <div className="text-[11px] font-semibold text-slate-400">
                              ({p.totalExpiredPcs.toLocaleString()} PCS)
                            </div>
                          )}
                        </td>
                        <td className="py-3 px-4 text-center">
                          <Badge variant="outline" className="border-slate-200 bg-slate-50 text-slate-700 font-bold text-[10px]">
                            EXPIRED
                          </Badge>
                        </td>
                      </tr>

                      {/* Collapsible Batch Breakdown Sub-Table */}
                      {isExpanded && (
                        <tr className="bg-slate-50/80">
                          <td colSpan={6} className="p-0 border-b border-blue-100">
                            <div className="p-3 pl-12">
                              <div className="border border-blue-100 rounded-lg overflow-hidden bg-white shadow-2xs">
                                <table className="w-full text-left text-xs">
                                  <thead className="bg-gradient-to-r from-blue-50 to-slate-50 text-[#2B4C86] font-bold uppercase tracking-wider border-b border-blue-100">
                                    <tr>
                                      <th className="py-2.5 px-3">Batch No</th>
                                      <th className="py-2.5 px-3">MFD</th>
                                      <th className="py-2.5 px-3">UBD</th>
                                      <th className="py-2.5 px-3 text-center">UBD %</th>
                                      <th className="py-2.5 px-3 text-right font-black text-slate-800">
                                        Expired Stock
                                      </th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-blue-100">
                                    {hasBatches ? (
                                      p.batches.map((b, idx) => {
                                        const ubdVal = b.ubd || b.expiryDate || null
                                        const pct = calcUBDPercent(ubdVal, b.shelfLifeDays, b.manufacturingDate)
                                        return (
                                          <tr key={idx} className="hover:bg-slate-50">
                                            <td className="py-2 px-3 font-mono font-bold text-slate-800">
                                              <Badge
                                                variant="outline"
                                                className="border-blue-200 bg-blue-50/60 text-[#2B4C86] font-mono text-[11px]"
                                              >
                                                {b.batchNumber}
                                              </Badge>
                                            </td>
                                            <td className="py-2 px-3 text-slate-500 font-medium">
                                              {b.manufacturingDate || "—"}
                                            </td>
                                            <td className="py-2 px-3 text-slate-600 font-medium">
                                              {ubdVal || "—"}
                                            </td>
                                            <td className="py-2 px-3 text-center">
                                              <Badge variant="outline" className="border-slate-200 bg-slate-50 text-slate-700 font-bold text-[10px]">
                                                EXPIRED
                                              </Badge>
                                            </td>
                                            <td className="py-2 px-3 text-right font-black text-sm text-slate-800">
                                              <div className="font-extrabold text-slate-800">{b.expiredQtyDisplay}</div>
                                              {p.pcsPerCrt > 1 && (
                                                <div className="text-[10px] font-semibold text-slate-400">
                                                  ({b.expiredQtyTotal.toLocaleString()} PCS)
                                                </div>
                                              )}
                                            </td>
                                          </tr>
                                        )
                                      })
                                    ) : (
                                      <tr>
                                        <td colSpan={5} className="py-3 px-4 text-center text-slate-400 font-medium">
                                          No individual batch entry recorded.
                                        </td>
                                      </tr>
                                    )}
                                  </tbody>
                                </table>
                              </div>
                            </div>
                          </td>
                        </tr>
                      )}
                    </Fragment>
                  )
                }))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </div>
  )
}
